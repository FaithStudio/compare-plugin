...
std::pair<int, int> jumpToNextChange(int mainStartLine, int subStartLine, bool down, bool doNotBlink = false)
{
	...
	else if (otherLine >= 0)
	{
		...
		const int otherVisibleLine	= CallScintilla(view, SCI_VISIBLEFROMDOCLINE, otherLine, 0);
	...
	if (Settings.FollowingCaret)
	{
		...
		const int pos = (down ?
				CallScintilla(view,
						((CallScintilla(view, SCI_ANNOTATIONGETLINES, line, 0) > 0) &&
						(CallScintilla(view, SCI_WRAPCOUNT, line, 0) > 1)) ?
						SCI_GETLINEENDPOSITION : SCI_POSITIONFROMLINE, line, 0) :
				CallScintilla(view, SCI_POSITIONFROMLINE, line, 0));
...
std::pair<int, int> jumpToChange(bool down, bool wrapAround)
{
	...
	if (Settings.FollowingCaret)
	{
		...
		{
			mainStartLine	= getCurrentLine(MAIN_VIEW);
			subStartLine	= otherViewMatchingLine(MAIN_VIEW, mainStartLine);
		}
		else
		{
			subStartLine	= getCurrentLine(SUB_VIEW);
			mainStartLine	= otherViewMatchingLine(SUB_VIEW, subStartLine);
		}
	}
